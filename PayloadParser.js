function parseUplink(device, payload) {
    ...
    // Create an input object with the required fields for decodeUplink
    var input = {
        bytes: data,
        fPort: null, // Use null or specify the correct fPort if available
        variables: {} // Provide any required variables or use an empty object
    };

    // Call decodeUplink with the correct input structure
    var decoded = decodeUplink(input).data;
    env.log('Data Decodificada: ', decoded);

    // Check if decoded data is valid before updating device endpoints
    var e = device.endpoints.byAddress("1");
    if (e != null && decoded.af_mps !== undefined) {
        e.updateGenericSensorStatus(decoded.af_mps);
    }

    e = device.endpoints.byAddress("2");
    if (e != null && decoded.dp_pa !== undefined) {
        e.updatePressureSensorStatus(decoded.dp_pa);
    }

    e = device.endpoints.byAddress("3");
    if (e != null && decoded.batt_mv !== undefined) {
        e.updateGenericSensorStatus(decoded.batt_mv);
        device.updateDeviceBattery({ voltage: decoded.batt_mv });
    }
}
